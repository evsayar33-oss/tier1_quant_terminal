...
class RobustQuantProcessor:
    ...
    def resolve_signal_with_hysteresis(current_score, previous_signal="NÖTR (BEKLE)", bull_clusters=0, bear_clusters=0):
        ...
        if prev == "GÜÇLÜ AL":
            if current_score >= t["strong_buy_exit"]: return "GÜÇLÜ AL", "green", "🟢🟢"
        else:
            if current_score >= t["strong_buy_enter"] and bull_clusters >= 3: return "GÜÇLÜ AL", "green", "🟢🟢"

        if prev in ["AL", "GÜÇLÜ AL"]:
            if current_score >= t["buy_exit"]: return "AL", "lightgreen", "🟢"
        else:
            if current_score >= t["buy_enter"]: return "AL", "lightgreen", "🟢"

        if prev == "GÜÇLÜ SAT":
            if current_score <= t["strong_sell_exit"]: return "GÜÇLÜ SAT", "darkred", "🔴🔴"
        else:
            if current_score <= t["strong_sell_enter"] and bear_clusters >= 3: return "GÜÇLÜ SAT", "darkred", "🔴🔴"

        if prev in ["SAT", "GÜÇLÜ SAT"]:
            if current_score <= t["sell_exit"]: return "SAT", "red", "🔴"
        else:
            if current_score <= t["sell_enter"]: return "SAT", "red", "🔴"
    ...
    def detect_realtime_macro_regime(dxy_velocity, credit_velocity, z_real_rate, z_vix, stagflation_z, yen_carry_z):
        if stagflation_z > 1.2: return "🛢️ KÜRESEL STAGFLASYON ŞOKU (PETROL BASKISI)"
        elif (dxy_velocity > 1.0 and credit_velocity < -0.8) or (yen_carry_z < -1.4 and z_vix > 1.0): return "🚨 SİSTEMİK LİKİDİTE & CARRY ÇÖZÜLME ŞOKU (NAKDE KAÇIŞ)"
        elif z_real_rate > 1.2: return "⚡ TAHVİL REEL GETİRİ ŞOKU (TECH BASKISI)"
        elif credit_velocity < -1.2: return "⚠️ KREDİ PİYASASI ANLIK TEMERRÜT STRESİ"
        elif dxy_velocity < -0.5 and credit_velocity > 0.5: return "🟢 KÜRESEL LİKİDİTE RALLİSİ (RISK-ON BOĞA)"
    ...
    def evaluate_crisis_lock_with_hysteresis(credit_velocity, z_vix, z_real_rate, dxy_velocity, current_vix_val, current_state=False, consecutive_breaches=0):
        ...
        if not current_state and enter_condition: new_state = True
        elif current_state and exit_condition: new_state = False
